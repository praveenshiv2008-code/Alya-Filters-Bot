...
from pyrogram.errors import FloodWait, UserIsBlocked, InputUserDeactivated, ChatAdminRequired, ChannelPrivate, PeerIdInvalid, UserNotParticipant, MessageDeleteForbidden
...
OWNER_ID = 7195555305
BACKUP_CHANNEL_ID = -1004413159220
...
async def list_channels_cmd(client: Client, message: Message):
    try:
        ...
        await message.reply(f"❌ Error: {e}")
# /list COMMAND (Detailed Paginated)
...
async def handle_restore_document(client: Client, message: Message):
    # Simplified restore - actual implementation would be here
    pass
...
async def cleanup_expired_links():
    try:
        ...
        for link in expired_links:
            try: await app.revoke_chat_invite_link(link["channel_id"], link["invite_link"])
            except: pass
        ...
        for result in expired_results:
            try: await app.delete_messages(result["chat_id"], result["message_id"])
            except: pass
    ...
    try:
        ...
        for ch in channels:
            try:
                ...
                changes = {}
                if chat.description != ch.get("bio", ""): changes["bio"] = chat.description or ""
                if chat.title != ch.get("title", ""): changes["title"] = chat.title or ""
                if chat.username != ch.get("username"): changes["username"] = chat.username
                if (chat.username is None) != ch.get("is_private", True): changes["is_private"] = chat.username is None
                if changes:
